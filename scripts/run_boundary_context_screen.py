"""Test longer intraday observations and strictly released label context."""
...
REPRESENTATIONS = ("observations", "released")
...
def run_fold(day, protocol, output, identity):
    ...
    forecasts = {}
    original_tree = joblib.load(source / "recent4_stride4_tree.joblib")
    original_neural = PooledForecaster.load(source / "recent4_stride4_neural")
    for asset, symbol in enumerate(SYMBOLS):
        ...
        np.testing.assert_array_equal(original_tree.predict_proba(test_x[symbol][original_tree.columns], symbol), forecasts[symbol]["recent4_stride4_tree"]["probabilities"])
        np.testing.assert_array_equal(original_neural.predict_proba(test_x[symbol][original_neural.columns], asset), forecasts[symbol]["recent4_stride4_neural"]["probabilities"])
        expected_priors = np.array([(training_y[symbol] == k).mean() for k in (-1, 0, 1)])
        np.testing.assert_array_equal(expected_priors, forecasts[symbol]["recent4_stride4_tree"]["train_priors"])
    del original_tree, original_neural
    metadata = {}
    for representation in REPRESENTATIONS:
        columns = context_columns(training_x[SYMBOLS[0]], representation)
        tx, vx = {s: training_x[s][columns] for s in SYMBOLS}, {s: validation_x[s][columns] for s in SYMBOLS}
        for kind in ("tree", "neural"):
            ...
            for asset, symbol in enumerate(SYMBOLS):
                ...
                p = restored.predict_proba(test_x[symbol][columns], identifier)
                np.testing.assert_array_equal(p, model.predict_proba(test_x[symbol][columns], identifier))
            ...
            print(f"context_fit={day}/{name} seconds={metadata[name]['seconds']:.1f}", flush=True)
            del model, restored
            gc.collect()
        for symbol in SYMBOLS:
            ...
            old_a, old_b = forecasts[symbol]["recent4_stride4_tree"], forecasts[symbol]["recent4_stride4_neural"]
...
def reveal(output, protocol, identity):
    ...
    if len(records) != protocol["post_fit_panels"]:
        raise ValueError("Incomplete intraday-context development family")
...
def run(protocol_path, output):
    ...
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Registered context input changed: {path}")
    ...
    if frozen.exists() and json.loads(frozen.read_text()) != identity:
        raise ValueError("Preserve the existing context experiment after source changes")
    write_json(frozen, identity)
    for count, day in enumerate(protocol["dates"], 1):
        record = run_fold(day, protocol, output, identity)
        print(f"context_dates_frozen={count}/{len(protocol['dates'])} date={day} seconds={record['seconds']:.1f}", flush=True)
        gc.collect()
    write_json(output / "summary.json", reveal(output, protocol, identity))
    print(f"context_development_complete {output / 'summary.json'}", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--protocol", type=Path, default=ROOT / "docs/research/boundary_context_screen_20260907.json")
    parser.add_argument("--output", type=Path, default=ROOT / "results/boundary_context_screen_20260907")
    parser.add_argument("--run", action="store_true")
    args = parser.parse_args()
    if args.run:
        run(args.protocol.resolve(), args.output.resolve())
    else:
        print("Pass --run for the fixed exposed-date intraday-context experiment.")
