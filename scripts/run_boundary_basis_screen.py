"""Test causal detrending of venue price bases with fixed predictive learners."""
...
from lob_forge.boundary_basis_inputs import load_basis_inputs, original_basis_columns
...
BASELINES = ("original_reference", "original_blend", "observations_blend", "spot100_tree",
             "combined_hgb", "combined_shallow_balanced", "combined_deep_balanced", "combined_deep_natural",
             "combined_deep_balanced_old_neural", "top1_100_tree_old_neural", "observations_neural")
MODEL_SPECS = {"combined_neural": ("combined", "neural"), "basis_neural": ("basis", "neural"),
               "basis_hgb": ("basis", "hgb"), "basis_xgb": ("basis", "xgb")}
BLENDS = {"combined_blend": ("combined_hgb", "combined_neural"),
          "basis_blend": ("basis_hgb", "basis_neural"),
          "basis_hgb_old_neural": ("basis_hgb", "observations_neural"),
          "basis_xgb_hgb": ("basis_xgb", "basis_hgb"),
          "basis_xgb_old_neural": ("basis_xgb", "observations_neural"),
          "basis_neural_combined_hgb": ("basis_neural", "combined_hgb")}
...
def run_fold(day, protocol, output, identity):
    ...
    source = ROOT / protocol["newton_run"] / "dates" / day
    context_source = ROOT / protocol["context_run"] / "dates" / day
    depth_source = ROOT / protocol["depth_run"] / "dates" / day
    for prior_source in (source, context_source, depth_source):
    ...
    for current in [*training_days, validation_day, day]:
        ...
        x, y, times = load_basis_inputs(ROOT, partition, ROOT / protocol["depth_manifest"], ROOT / protocol["spot_manifest"])
    ...
    combined_hgb = joblib.load(source / "combined_hgb.joblib")
    combined_xgb = NewtonBoostForecaster.load(source / "combined_deep_balanced")
    for asset, symbol in enumerate(SYMBOLS):
        forecasts[symbol] = {}
        for name in BASELINES:
            parent = depth_source if name == "top1_100_tree_old_neural" else source
        ...
        for parent, name in ((combined_hgb, "combined_hgb"), (combined_xgb, "combined_deep_balanced")):
            np.testing.assert_array_equal(parent.predict_proba(test_x[symbol][parent.columns], symbol), forecasts[symbol][name]["probabilities"])
    ...
    del original_tree, original_neural, combined_hgb, combined_xgb
    metadata = {}
    for name, (representation, kind) in MODEL_SPECS.items():
        columns = original_basis_columns(training_x[SYMBOLS[0]]) if representation == "combined" else list(training_x[SYMBOLS[0]].columns)
        tx = {s: training_x[s][columns] for s in SYMBOLS}
        vx = {s: validation_x[s][columns] for s in SYMBOLS}
        ...
        if kind == "hgb":
            ...
            model = fit_newton_boost(tx, training_y, max_depth=6, class_balanced=True)
        ...
        print(f"basis_fit={day}/{name} seconds={metadata[name]['seconds']:.1f}", flush=True)
        del model, restored
        gc.collect()
    for symbol in SYMBOLS:
...
def reveal(output, protocol, identity):
    ...
    if len(records) != protocol["post_fit_panels"]:
        raise ValueError("Incomplete causal basis-state development family")
...
def run(protocol_path, output):
    ...
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Registered causal basis-state input changed: {path}")
    identity = {"protocol_sha256": sha256_file(protocol_path), "input_hashes": protocol["input_hashes"], "python": platform.python_version(),
                "dependencies": {name: version(name) for name in ("numpy", "pandas", "scikit-learn", "torch", "pyarrow", "scipy", "xgboost")}}
    frozen = output / "frozen_screen.json"
    if frozen.exists() and json.loads(frozen.read_text()) != identity:
        raise ValueError("Preserve the existing causal basis-state experiment after source changes")
    write_json(frozen, identity)
    for count, day in enumerate(protocol["dates"], 1):
        record = run_fold(day, protocol, output, identity)
        print(f"basis_dates_frozen={count}/{len(protocol['dates'])} date={day} seconds={record['seconds']:.1f}", flush=True)
        gc.collect()
    write_json(output / "summary.json", reveal(output, protocol, identity))
    print(f"basis_development_complete {output / 'summary.json'}", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--protocol", type=Path, default=ROOT / "docs/research/boundary_basis_screen_20260907.json")
    parser.add_argument("--output", type=Path, default=ROOT / "results/boundary_basis_screen_20260907")
    parser.add_argument("--run", action="store_true")
    args = parser.parse_args()
    if args.run:
        run(args.protocol.resolve(), args.output.resolve())
    else:
        print("Pass --run for the fixed exposed-date causal basis-state experiment.")
