"""Compare independent spot trades, matched perpetual trade features and information delay."""
...
from lob_forge.boundary_spot_inputs import VARIANTS, load_spot_inputs, select_spot_variant
...
BASELINES = ("original_reference", "original_blend", "clock_neural_clock_tree_7",
             "observations_tree", "observations_neural", "observations_blend")
...
def run_fold(day, protocol, output, identity):
    ...
    source = ROOT / protocol["context_run"] / "dates" / day
    ...
    for current in [*training_days, validation_day, day]:
        ...
        x, y, times = load_spot_inputs(ROOT, partition, ROOT / protocol["spot_manifest"])
    ...
    forecasts = {}
    original_tree = joblib.load(source / "observations_tree.joblib")
    original_neural = PooledForecaster.load(source / "observations_neural")
    ...
    for representation in REPRESENTATIONS:
        tx = {s: select_spot_variant(training_x[s], representation) for s in SYMBOLS}
        vx = {s: select_spot_variant(validation_x[s], representation) for s in SYMBOLS}
        test_selected = {s: select_spot_variant(test_x[s], representation) for s in SYMBOLS}
        ...
        for kind in ("tree", "neural"):
            ...
            print(f"spot_fit={day}/{name} seconds={metadata[name]['seconds']:.1f}", flush=True)
            del model, restored
            gc.collect()
        for symbol in SYMBOLS:
...
def reveal(output, protocol, identity):
    ...
    if len(records) != protocol["post_fit_panels"]:
        raise ValueError("Incomplete auxiliary trade-source development family")
    ...
    return {"identity": identity, "evidence_status": protocol["evidence_status"], "model_fits": 6 * len(protocol["dates"]), "post_fit_panels": len(records),
...
def run(protocol_path, output):
    ...
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Registered auxiliary trade input changed: {path}")
    ...
    if frozen.exists() and json.loads(frozen.read_text()) != identity:
        raise ValueError("Preserve the existing auxiliary trade experiment after source changes")
    write_json(frozen, identity)
    for count, day in enumerate(protocol["dates"], 1):
        record = run_fold(day, protocol, output, identity)
        print(f"spot_dates_frozen={count}/{len(protocol['dates'])} date={day} seconds={record['seconds']:.1f}", flush=True)
        gc.collect()
    write_json(output / "summary.json", reveal(output, protocol, identity))
    print(f"spot_development_complete {output / 'summary.json'}", flush=True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--protocol", type=Path, default=ROOT / "docs/research/boundary_spot_screen_20260907.json")
    parser.add_argument("--output", type=Path, default=ROOT / "results/boundary_spot_screen_20260907")
    parser.add_argument("--run", action="store_true")
    args = parser.parse_args()
    if args.run:
        run(args.protocol.resolve(), args.output.resolve())
    else:
        print("Pass --run for the fixed exposed-date auxiliary trade-source experiment.")
