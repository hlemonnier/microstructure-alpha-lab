"""Test fixed native-quote source ablations on unchanged prediction targets."""
...
from lob_forge.boundary_quote_currency_inputs import append_quote_sources, quote_variant_columns
...
VARIANTS = ("fx100", "btc100", "both100")
MODEL_SPECS = {f"{representation}_{variant}_{kind}": {"variant": variant, "representation": representation,
    "kind": kind, "leaves": 7 if kind == "hgb" else None}
    for representation in ("observations", "combined") for variant in VARIANTS for kind in ("hgb", "neural")}
BLENDS = {}
for _representation in ("observations", "combined"):
    for _variant in VARIANTS:
        _prefix = f"{_representation}_{_variant}"
...
def model_columns(metadata, name):
    ...
    original = metadata["original_columns"] if spec["representation"] == "combined" else metadata["observation_columns"]
    return quote_variant_columns(original, spec["variant"])
...
def prepare_fold(day, protocol, folder, identity):
    ...
    sessions = {(r["symbol"], r["date"]): r for r in feature_manifest["sessions"]}
    partitions = {}
    for current, entries in original_metadata["partitions"].items():
        additions = {}
        for symbol in SYMBOLS:
            record = sessions[symbol, current]
            for delay in (100, 500):
                group = record["groups"][str(delay)]
                path = ROOT / group["features_path"]
                if sha256_file(path) != group["sha256"]:
                    raise ValueError("An alternative-quote cache changed")
                frame = pd.read_parquet(path)
                if list(frame.columns) != group["columns"] or len(frame) != record["decision_rows"]:
                    raise ValueError("Original alternative-quote cache dimensions changed")
                additions[symbol, delay] = frame.set_index("decision_time")
        partitions[current] = {}
        for symbol in SYMBOLS:
            entry = entries[symbol]
            times = np.load(source / entry["times"], allow_pickle=False)
            original = pd.read_parquet(source / entry["features"], columns=original_metadata["original_columns"])
            ...
            augmented = append_quote_sources(original, times, additions)
            # Store only predeclared 100 ms model columns; reading a 500 ms cache
            # above validates input alignment, but cannot add it to any learner.
            columns = quote_variant_columns(original_metadata["original_columns"], "both100")
            augmented = augmented[columns]
...
def fit_worker(protocol_path, output, day, name):
    ...
    if protocol["model_specs"] != MODEL_SPECS or day not in protocol["dates"]:
        raise ValueError("Only the frozen quote-source model family may be fitted")
    ...
    with threadpool_limits(limits=2):
        ...
        for asset, symbol in enumerate(SYMBOLS):
            ...
            if errors["future_prefix"] > protocol["future_prefix_absolute_tolerance"] or max(errors.values()) > protocol["query_partition_absolute_tolerance"]:
                raise ValueError(f"Quote-source model query dependence exceeded its fixed tolerance: {errors}")
...
def reveal(protocol, output, identity):
    ...
    if len(records) != protocol["post_fit_panels"]:
        raise ValueError("Every fixed quote-source model reporting panel must be present")
    leaderboard = []
    for name in (*BASELINES, *MODEL_SPECS, *BLENDS):
        for policy in ("registered", "forecast_3600"):
...
def run(protocol_path, output, prepare_only=False):
    ...
    if (protocol["model_specs"] != MODEL_SPECS or protocol["baselines"] != list(BASELINES)
        ...
        raise ValueError("The exact registered quote-source model family is required")
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Frozen quote-source model input changed: {path}")
    if output.exists():
        raise ValueError("Preserve every previous quote-source model attempt; no implicit resume")
    ...
    for day in protocol["dates"]:
        ...
        print(f"quote_inputs_verified={day}", flush=True)
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Input changed before quote-source model fitting: {path}")
    if prepare_only:
    ...
    for day in protocol["dates"]:
        ...
        for name in MODEL_SPECS:
            ...
            if returncode != 0 or failure is not None:
                raise ValueError(f"Quote-source model worker failed; preserve its attempt: {day}/{name}")
            print(f"quote_fit={day}/{name} seconds={supervision['seconds']:.1f}", flush=True)
        gc.collect()
    for path, checksum in protocol["input_hashes"].items():
        if sha256_file(ROOT / path) != checksum:
            raise ValueError(f"Input changed during quote-source model execution: {path}")
    for day in protocol["dates"]:
