...
The expected-edge study is complete only when `verify_expected_edge_study.sh` reports `complete=1`. The verifier reads `run_plan.json`, expands every expected symbol/horizon/fee result pair, checks the result CSV, audit CSV, `pvalues.csv`, and `pvalue_corrections.csv`, then runs the artifact verifier. The stricter remaining-evidence gate also requires at least 20 audit folds per artifact. For the full profile that means 80 result CSVs and 80 audit CSVs. For the laptop quick profile it means 10 result CSVs and 10 audit CSVs.
