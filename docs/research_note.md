...
Adjacent folds, overlapping label windows, and repeated high-frequency trades are not iid observations. Reports should aggregate by day/fold/regime where possible and should state when exact selection-adjusted inference is infeasible. `audit-results` now declares its `inference_grain` explicitly; the current walk-forward CSV artifacts are fold-summary audits, while the reduced E2E fixture derives its intervals from simulator ledgers. `verify-results` rejects unsupported inference-grain labels, rejects trade/day/position ledger-grain claims unless the audit row points to a present ledger artifact, requires ordinary non-config-hash `pvalues.csv` files to cover the audit artifact family exactly, and rejects in-repository experiment ledgers whose `git_rev` does not match the current checkout `HEAD`, has no local tag, or whose `working_tree_dirty` provenance is missing, non-boolean, or true. Larger empirical runs must preserve trade/day ledgers beside fold summaries before claiming trade- or day-level inference. The reduced E2E run writes a machine-readable experiment registry with evaluated status and validation/test PnL for each predeclared threshold attempt; larger empirical runs must do the same for every attempted feature, horizon, cost, model class, hyperparameter, and seed.
