...
The quote/trade builder decides at the deterministic close of a completed `[start, close)` bucket. It retains the latest quote strictly before close, sums OFI across every raw transition, and admits only trades/depth snapshots strictly before close. The trailing partial bucket is omitted. Old last-quote-time decisions and bucket-retained future execution are unsupported.

Entry/exit resolve on the first raw quote at or after `decision+latency` and `decision+latency+horizon`. Update IDs prevent tied-timestamp execution before the quote used for prediction. The CSV preserves decision/quote/entry/future timestamps, update IDs, lag measurements and actual execution-quote quantities. These are exchange-event-time assumptions; local receive timestamps remain unavailable when the source did not record them. Sparse quote gaps can make the actual holding interval differ from its nominal horizon; inspect the reported lags.

Tests check completed-boundary scheduling, raw-event OFI, post-decision feature invariance and multiple updates at one millisecond. See [the September remediation](model_math_remediation.md) for the complete mathematical contracts and limitations.
...
The simulator supports maker/taker paths, top-of-book and L2-level walking, partial fills, fees, slippage, latency, expiry checks, position limits, leverage limits, rate limiting, passive queue-ahead assumptions, cancel/replace on signal decay, and a kill switch that prevents subsequent orders. Passive orders are post-only: marketable limits are rejected rather than mislabeled as maker fills, default passive prices rest on the same-side best quote, and a newly submitted order cannot consume trade flow with the same timestamp. Portfolio risk is also evaluated in event order: exit PnL reaches kill switches only at exit time, signed net inventory is distinct from gross exposure, and short positions are handled symmetrically. Observed-fill calibration remains an external paper/live validation task.
...
Adjacent folds, overlapping label windows, and repeated high-frequency trades are not iid observations. Reports should aggregate by day/fold/regime where possible and should state when exact selection-adjusted inference is infeasible. `audit-results` declares its `inference_grain`; current walk-forward artifacts are fold-summary audits, while the reduced E2E fixture derives intervals from simulator ledgers. For expected-edge grids, thresholds are candidates inside one validation-selection procedure. The audit p-value therefore appears once per completed procedure and its `procedure_sha256` binds the full threshold/model/feature grid and selection rule. Replicating that p-value onto every threshold is rejected. Larger empirical runs must preserve trade/day ledgers and the complete attempted search registry before making trade-, day-, or selection-adjusted claims.
...
| Diagnostic gross edge | legacy local docs/results under ignored `results/` | historical only | Useful exploration, but not promotable until regenerated with verified feature manifests and result provenance sidecars. |
...
| Neural smoke | `sequence_tcn_smoke.csv`, `sequence_transformer_smoke.csv` | stale-contract | Prior tiny-fixture outputs are retained locally but fail the new version/hash/inventory checks until cleanly regenerated. |
