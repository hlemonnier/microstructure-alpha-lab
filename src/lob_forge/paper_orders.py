...
def _instruction_for_decision(
    ...
    *,
    ...
    if provider == "bybit":
        return _bybit_instruction(decision, category=category, time_in_force=time_in_force)
    if provider == "okx":
        return _okx_instruction(decision, td_mode=td_mode)
    if provider == "binance":
        return _binance_instruction(decision, time_in_force=time_in_force)
    raise ValueError(f"provider must be one of: {', '.join(SUPPORTED_PAPER_ORDER_PLAN_PROVIDERS)}")
...
def _okx_instruction(decision: ShadowDecision, *, td_mode: str) -> PaperOrderInstruction:
    payload: dict[str, object] = {
...
def _binance_instruction(decision: ShadowDecision, *, time_in_force: str) -> PaperOrderInstruction:
    payload: dict[str, object] = {
        "symbol": decision.symbol,
        "side": "BUY" if decision.predicted_side > 0 else "SELL",
