...
The ongoing [accuracy research log](docs/research/boundary_research_log.md) records the later model experiments, exact target arithmetic and independent-date gate. Its first independent confirmation fell short of the substantial-gain threshold. Subsequent richer-feature and model comparisons remain development evidence; the [completed pretrained-transformer study](docs/research/boundary_tabicl_report_20260908.md) improves over its small-context tree comparator but does not beat the strongest full-data control. The [continuous fixed-memory comparison](docs/research/boundary_fading_memory_report_20260908.md) also finds no substantial gain: its best blend improves the development mean by only 0.0333 percentage points and uses instantaneous features. The [feature-budget and larger-context experiment](docs/research/boundary_tabicl_feature_budget_report_20260908.md) improves its transformer component by 0.3979 points, but every new complete procedure remains below the retained control. The [historical learned-retrieval experiment](docs/research/boundary_retrieval_report_20260908.md) reaches 57.2976% in its best blend, below the retained 57.3408%; large gains over fixed-distance retrieval do not carry over to the strongest complete procedure.
