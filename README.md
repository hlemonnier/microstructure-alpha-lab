...
The ongoing [accuracy research log](docs/research/boundary_research_log.md) records the later model experiments, exact target arithmetic and independent-date gate. Its first independent confirmation fell short of the substantial-gain threshold. Subsequent richer-feature and model comparisons remain development evidence; the [completed pretrained-transformer study](docs/research/boundary_tabicl_report_20260908.md) improves over its small-context tree comparator but does not beat the strongest full-data control.
